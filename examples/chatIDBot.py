...
def start(client: Client, message: types.Update):
...
@client.on_updateNewMessage(is_private_filter)
def handle_shared_chat(client: Client, message: types.Update):
...
def chat_member(client: Client, update: types.Update):
    if (
